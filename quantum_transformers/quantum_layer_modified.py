...
def phase_embedding(c: tc.Circuit, inputs):
    num_qubits = inputs.shape[-1]
    for j in range(num_qubits):
        c.h(j)
        c.rz(j, theta=inputs[j])
        c.h(j)

def amplitude_embedding(c: tc.Circuit, inputs):
    num_qubits = inputs.shape[-1]
    # Normalize inputs to unit norm
    norm = jnp.linalg.norm(inputs)
    normalized = inputs / norm
    c.initialize(normalized, range(num_qubits))

def iqp_embedding(c: tc.Circuit, inputs):
    n = inputs.shape[-1]
    angles = [(x / 10) * jnp.pi for x in inputs]  # Assuming inputs are in [0, 10]
    for i in range(n):
        c.h(i)
    for i in range(n):
        c.rz(i, theta=angles[i])
    for i in range(n):
        for j in range(i + 1, n):
            c.cx(i, j)
            c.rz(j, theta=angles[i] * angles[j])
            c.cx(i, j)
